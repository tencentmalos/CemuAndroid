...
- foundation 植入与调整：
  ...
  - 当前状态：C1、C2 已完成；本地/内部镜像 `main` 与官方 `upstream/main` 均为 `b8f2cf4b`，并已由 `fc884596` 完整 merge 到 `feature/malos/basic_version`。foundation 仍钉在 `b01f41c`，根 CMake 已正规化接入。C2 后续的依赖收敛已把 fmt、glslang、zstd、libusb、Crypto++ 改为 `tencentmalos` 子模块，并复用 foundation 的 RapidJSON；没有删除后续要用的 core、network、profiler、XR 能力。C3-T1 已完成，RelWithDebInfo 默认关闭 LTO、Release 保留 LTO；验证见 `docs/verification/20260726-C3/build-speedup-notes.md`，下一步是 C3-T2 ccache。
